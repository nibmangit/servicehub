    ...
    <aside className="hidden w-64 shrink-0 border-r border-border bg-card lg:block">
      <div className="sticky top-0 flex h-[calc(100vh-4rem)] flex-col p-4">
        <Section label={t("side.menu")}>
          {primary.map((i) => <NavItem key={i.to} item={i} active={isActive(i.to)} />)}
        </Section>
        <Section label={t("side.provider")}>
          {provider.map((i) => <NavItem key={i.to} item={i} active={isActive(i.to)} />)}
        </Section>
        <Section label={t("side.account")}>
          {account.map((i) => <NavItem key={i.to} item={i} active={isActive(i.to)} />)}
        </Section>

        <div className="mt-auto rounded-xl border border-border bg-secondary/40 p-3">
          <div className="flex items-center gap-3">
            <Avatar className="h-9 w-9">
              <AvatarImage src="https://i.pravatar.cc/120?u=me" alt="Me" />
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="truncate text-xs text-muted-foreground">Addis Ababa</div>
            </div>
            <Link
              to="/"
